...
def search_transcripts(text):
    return mongo_utils.to_json(transcript_service.search_transcripts(text, request.args.get('episode_number'),
                                                                     request.args.get('actor_nickname')))


@transcript_controller.route('/api/transcripts/search_per_episode/<text>')
@cross_origin()
def search_transcripts_per_episode(text):
    return send_file(transcript_service.search_per_episode(text, request.args.get('actor_nickname')),
                     mimetype='image/png')
...
def wordcloud():
    return send_file(
        transcript_service.wordcloud(request.args.get('actor_nickname'), request.args.get('episode_number')),
        mimetype='image/png')
...
def rank_actors_by_words(words):
    return send_file(transcript_service.rank_actors_by_words(words), mimetype='image/png')
